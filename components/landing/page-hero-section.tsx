    ...
    <div className={`transition-all duration-700 ${vis ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
      ...
      {titulo && (
        <StudioInlineText
          ...
          style={{ color: titleColor, fontSize: `clamp(${Math.max(34, Math.round(titleSize * 0.58))}px, 5.2vw, ${titleSize}px)` }}
          editorStyle={{ color: titleColor, fontSize: `clamp(${Math.max(30, Math.round(titleSize * 0.48))}px, 4.8vw, ${Math.max(36, titleSize - 8)}px)` }}
        />
      )}
      {subtitulo && (
        <StudioInlineText
